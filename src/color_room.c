...
t_color	color_room(t_ray ray, t_room room, int depth)
{
	...
	if (hit_obj(&rec, ray, room))
	{
		...
		if (rec.material == MATTE)
		{
			...
			target = vec3_add(rec.hitpos, random_in_hemisphere(rec.normal));
		...
		if (rec.material == MIRROR)
		{
			vec3_normalize(&ray.direction);
			vec3_normalize(&rec.normal);
			reflected_dir = reflected_direction(ray.direction, rec.normal);
			t_ray reflected_ray = new_ray(rec.hitpos, reflected_dir);
			bool ref = (vec3_dot(reflected_ray.direction, rec.normal) > 0) ? true : false;
			if (ref)
			{
				color = color_room(reflected_ray, room, depth-1);
